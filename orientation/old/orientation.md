...
\*\*One action per `hao`.\*\* Each `hao` unlocks exactly one Write, Edit, or Bash call. After that call completes, stop and wait for the next `hao`. Never queue multiple write tools in a single response. Never assume `hao` carries forward to subsequent tool calls within the same turn.



\*\*Ask before acting.\*\* After completing an action, ask "Can I \[next specific action]?" and wait for `hao`. Do not announce what you will do next — request permission for it.
